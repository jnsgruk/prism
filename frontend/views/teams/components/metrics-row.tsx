...
export const MetricsRow = ({
  ...
  return (
    <TableRow className="cursor-pointer hover:bg-muted/50" onClick={() => navigate(`/teams/${metrics.teamId}`)}>
      ...
      {showDiscourse && <TableCell className="tabular-nums">{metrics.discourseTopicsCreated || "\u2014"}</TableCell>}
      {showDiscourse && <TableCell className="tabular-nums">{metrics.discoursePosts || "\u2014"}</TableCell>}
    </TableRow>
  );
};
