...
export const StatusDot = ({ state, animate }: { state: string; animate: boolean }): React.ReactElement => {
  const color = stateStyles[state]?.color ?? "bg-emerald-500";
  return (
    <span className="relative flex size-2.5">
      {animate && <span className={cn("absolute inline-flex size-full animate-ping rounded-full opacity-75", color)} />}
